...
if the product_qty_type is “unit,” 
...
SELECT product_id, 
       product_name,
       /* Previous logic from Query 5 */
       CASE 
           WHEN product_qty_type = 'unit' THEN 'unit'
           ELSE 'bulk' 
       END AS prod_qty_type_condensed,
       -- ! New column: check  "pepper"
       CASE 
...
SELECT *
	FROM vendor AS v
INNER JOIN vendor_booth_assignments as v_assign
	ON v.vendor_id = v_assign.vendor_id
...
--QUERY 8
SELECT vendor_id, count(*) as num_booth_assignments
...
INNER JOIN customer AS c_info
    ON c_info.customer_id = cp.customer_id
GROUP BY 
    cp.customer_id, 
    c_info.customer_first_name, 
    c_info.customer_last_name
HAVING SUM(cp.quantity * cp.cost_to_customer_per_qty) > 2000
ORDER BY c_info.customer_last_name, c_info.customer_first_name;
...
SELECT 
    customer_id, 
    SUM(quantity * cost_to_customer_per_qty) AS total_spent_apr2022
FROM (
    -- From Query 11
    SELECT *, 
           strftime('%m', market_date) AS month, 
